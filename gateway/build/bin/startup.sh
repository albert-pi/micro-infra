...
pid() {
  echo "$(ps -ef | grep microinfra.gateway | grep java | grep -v grep | awk '{print $2}')"
}
...
if [ -n "$(pid)" ]; then
  echo "gateway already started: $(pid)"
  exit 1
fi
...
GATEWAY_JVM_OPTS="$GATEWAY_JVM_OPTS \
...
# GC options
GATEWAY_JVM_OPTS="$GATEWAY_JVM_OPTS -XX:+UnlockExperimentalVMOptions \
...
# GATEWAY_JVM_OPTS="${GATEWAY_JVM_OPTS} -Xss4m"

#
# Uncomment to set preference for IPv4 stack.
# GATEWAY_JVM_OPTS="${GATEWAY_JVM_OPTS} -Djava.net.preferIPv4Stack=true"

#
# Remote debugging (JPDA). Uncomment and change if remote debugging is required.
# GATEWAY_JVM_OPTS="-agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=8787 ${JVM_OPTS}"
...
do
    if [ -d $file ] ; then
        GATEWAY_LIBS=${GATEWAY_LIBS:-}$SEP$file/*
    fi
done
...
nohup $JAVA_CMD $GATEWAY_JVM_OPTS -cp "$BASE_DIR/etc/;$GATEWAY_LIBS" -Dfile.encoding=UTF-8 -Dapp.home="$BASE_DIR" com.microinfra.gateway.GatewayApplication microinfra.gateway >/dev/null 2>&1 &

PIDS=$(pid)
echo "gateway($PIDS) started，you can check the state from log"
