...
pid() {
  echo "$(ps -ef | grep microinfra.app | grep java | grep -v grep | awk '{print $2}')"
}
...
if [ -n "$(pid)" ]; then
  echo "application already started: $(pid)"
  exit 1
fi
...
# Perf options
APPLICATION_JVM_OPTS="-server \
	-XX:MaxInlineLevel=15 \
	-Djava.awt.headless=true \
	"

# Heap options
# In general, heap memory is 70% of total memory
MEMORY=$(total_memory)
...
APPLICATION_JVM_OPTS="$APPLICATION_JVM_OPTS \
...
echo "Heap Memory: $(memory_in_mb $HEAP_MEMORY)m, Max Metaspace: $MAX_META_SPACE_MEMORY"
	
# GC options
APPLICATION_JVM_OPTS="$APPLICATION_JVM_OPTS -XX:+UnlockExperimentalVMOptions \
...
# APPLICATION_JVM_OPTS="${APPLICATION_JVM_OPTS} -Xss4m"

#
# Uncomment to set preference for IPv4 stack.
# APPLICATION_JVM_OPTS="${APPLICATION_JVM_OPTS} -Djava.net.preferIPv4Stack=true"

#
# Remote debugging (JPDA). Uncomment and change if remote debugging is required.
# APPLICATION_JVM_OPTS="-agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=8787 ${JVM_OPTS}"
...
do
    if [ -d $file ] ; then
        APPLICATION_LIBS=${APPLICATION_LIBS:-}$SEP$file/*
    fi
done
...
nohup $JAVA_CMD $APPLICATION_JVM_OPTS -cp "$BASE_DIR/etc/;$APPLICATION_LIBS" -Dfile.encoding=UTF-8 -Dapp.home="$BASE_DIR"  com.microinfra.application.StandaloneApplication microinfra.app >/dev/null 2>&1 &

PIDS=$(pid)
echo "application($PIDS) started，you can check the state from log"
