...
set /a MEM_LIMIT = 2*1024*1024
call :total_memory_in_kb MEMORY %MEM_LIMIT%

rem Perf options
set APPLICATION_JVM_OPTS=^
	-server ^
	-XX:MaxInlineLevel=15 ^
	-Djava.awt.headless=true

rem GC options
set APPLICATION_JVM_OPTS=%APPLICATION_JVM_OPTS% ^
...
rem set APPLICATION_JVM_OPTS=%APPLICATION_JVM_OPTS% -Xss16m

rem Uncomment to set preference to IPv4 stack.
rem set APPLICATION_JVM_OPTS=%APPLICATION_JVM_OPTS% -Djava.net.preferIPv4Stack=true

rem Uncomment to enable reverse DNS lookup.
rem set APPLICATION_JVM_OPTS=%APPLICATION_JVM_OPTS% -Dsun.net.spi.nameservice.provider.1=default -Dsun.net.spi.nameservice.provider.2=dns,sun

rem Remote debugging (JPDA). Uncomment and change if remote debugging is required.
rem set APPLICATION_JVM_OPTS=%APPLICATION_JVM_OPTS% -agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=8787 

set APPLICATION_LIBS=%BASE_DIR%\lib\*

for /F %%F in ('dir /A:D /b "%APPLICATION_LIBS%"') do (
	if not "%%F" == "optional" (
		set APPLICATION_LIBS=%APPLICATION_LIBS%;%BASE_DIR%\lib\%%F\*
	)
)
...
%JAVA_COMMAND% %APPLICATION_JVM_OPTS% -cp "%BASE_DIR%\etc\;%APPLICATION_LIBS%" -Dfile.encoding=UTF-8 -Dapp.home="%BASE_DIR%" com.microinfra.application.StandaloneApplication microinfra.app %*
