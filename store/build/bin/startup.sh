...
pid() {
  echo "$(ps -ef | grep microinfra.store | grep java | grep -v grep | awk '{print $2}')"
}
...
if [ -n "$(pid)" ]; then
  echo "store service already started: $(pid)"
  exit 1
fi
...
# GC options
STORE_JVM_OPTS="$STORE_JVM_OPTS -XX:+UnlockExperimentalVMOptions \
...
# STORE_JVM_OPTS="${STORE_JVM_OPTS} -Xss4m"

#
# Uncomment to set preference for IPv4 stack.
# STORE_JVM_OPTS="${STORE_JVM_OPTS} -Djava.net.preferIPv4Stack=true"

#
# Remote debugging (JPDA). Uncomment and change if remote debugging is required.
# STORE_JVM_OPTS="-agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=8787 ${JVM_OPTS}"
...
do
    if [ -d $file ] ; then
        STORE_LIBS=${STORE_LIBS:-}$SEP$file/*
    fi
done
...
nohup $JAVA_CMD $STORE_JVM_OPTS -cp "$BASE_DIR/etc/;$STORE_LIBS" -Dfile.encoding=UTF-8 -Dapp.home="$BASE_DIR" com.microinfra.store.StoreApplication microinfra.store >/dev/null 2>&1 &

PIDS=$(pid)
echo "store service($PIDS) started，you can check the state from log"
