...
pid() {
  echo "$(ps -ef | grep microinfra.admin | grep java | grep -v grep | awk '{print $2}')"
}
...
if [ -n "$(pid)" ]; then
  echo "system administration already started: $(pid)"
  exit 1
fi
...
# GC options
ADMIN_JVM_OPTS="$ADMIN_JVM_OPTS -XX:+UnlockExperimentalVMOptions \
...
# ADMIN_JVM_OPTS="${ADMIN_JVM_OPTS} -Xss4m"

#
# Uncomment to set preference for IPv4 stack.
# ADMIN_JVM_OPTS="${ADMIN_JVM_OPTS} -Djava.net.preferIPv4Stack=true"

#
# Remote debugging (JPDA). Uncomment and change if remote debugging is required.
# ADMIN_JVM_OPTS="-agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=8787 ${JVM_OPTS}"
...
do
    if [ -d $file ] ; then
        ADMIN_LIBS=${ADMIN_LIBS:-}$SEP$file/*
    fi
done
...
nohup $JAVA_CMD $ADMIN_JVM_OPTS -cp "$BASE_DIR/etc/;$ADMIN_LIBS" -Dfile.encoding=UTF-8 -Dapp.home="$BASE_DIR"  com.microinfra.admin.AdminApplication microinfra.admin >/dev/null 2>&1 &

PIDS=$(pid)
echo "system administration($PIDS) started，you can check the state from log"
