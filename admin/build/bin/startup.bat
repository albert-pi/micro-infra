...
call "%~dp0pid.bat" PID microinfra.admin
if defined PID (
...
rem GC options
set ADMIN_JVM_OPTS=%ADMIN_JVM_OPTS% ^
...
rem set ADMIN_JVM_OPTS=%ADMIN_JVM_OPTS% -Xss16m

rem Uncomment to set preference to IPv4 stack.
rem set ADMIN_JVM_OPTS=%ADMIN_JVM_OPTS% -Djava.net.preferIPv4Stack=true

rem Uncomment to enable reverse DNS lookup.
rem set ADMIN_JVM_OPTS=%ADMIN_JVM_OPTS% -Dsun.net.spi.nameservice.provider.1=default -Dsun.net.spi.nameservice.provider.2=dns,sun

rem Remote debugging (JPDA). Uncomment and change if remote debugging is required.
rem set ADMIN_JVM_OPTS=%ADMIN_JVM_OPTS% -agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=8787 

set ADMIN_LIBS=%BASE_DIR%\lib\*

for /F %%F in ('dir /A:D /b "%ADMIN_LIBS%"') do (
	if not "%%F" == "optional" (
		set ADMIN_LIBS=%ADMIN_LIBS%;%BASE_DIR%\lib\%%F\*
	)
)
...
%JAVA_COMMAND% %ADMIN_JVM_OPTS% -cp "%BASE_DIR%\etc\;%ADMIN_LIBS%" -Dfile.encoding=UTF-8 -Dapp.home="%BASE_DIR%" com.microinfra.admin.AdminApplication microinfra.admin %*
