...
pid() {
  echo "$(ps -ef | grep microinfra.uid | grep java | grep -v grep | awk '{print $2}')"
}
...
if [ -n "$(pid)" ]; then
  echo "UID service already started: $(pid)"
  exit 1
fi
...
# GC options
UID_JVM_OPTS="$UID_JVM_OPTS -XX:+UnlockExperimentalVMOptions \
...
# UID_JVM_OPTS="${UID_JVM_OPTS} -Xss4m"

#
# Uncomment to set preference for IPv4 stack.
# UID_JVM_OPTS="${UID_JVM_OPTS} -Djava.net.preferIPv4Stack=true"

#
# Remote debugging (JPDA). Uncomment and change if remote debugging is required.
# UID_JVM_OPTS="-agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=8787 ${JVM_OPTS}"
...
nohup $JAVA_CMD $UID_JVM_OPTS -cp "$BASE_DIR/etc/;$UID_LIBS" -Dfile.encoding=UTF-8 -Dapp.home="$BASE_DIR" com.microinfra.uid.UidApplication microinfra.uid >/dev/null 2>&1 &

PIDS=$(pid)
echo "UID($PIDS) service started，you can check the state from log"
