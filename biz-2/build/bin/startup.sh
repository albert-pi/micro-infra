...
pid() {
  echo "$(ps -ef | grep microinfra.parking | grep java | grep -v grep | awk '{print $2}')"
}
...
if [ -n "$(pid)" ]; then
  echo "parking service already started: $(pid)"
  exit 1
fi
...
PARKING_JVM_OPTS="$PARKING_JVM_OPTS \
...
# GC options
PARKING_JVM_OPTS="$PARKING_JVM_OPTS -XX:+UnlockExperimentalVMOptions \
...
# PARKING_JVM_OPTS="${PARKING_JVM_OPTS} -Xss4m"

#
# Uncomment to set preference for IPv4 stack.
# PARKING_JVM_OPTS="${PARKING_JVM_OPTS} -Djava.net.preferIPv4Stack=true"

#
# Remote debugging (JPDA). Uncomment and change if remote debugging is required.
# PARKING_JVM_OPTS="-agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=8787 ${JVM_OPTS}"
...
do
    if [ -d $file ] ; then
        PARKING_LIBS=${PARKING_LIBS:-}$SEP$file/*
    fi
done
...
nohup $JAVA_CMD $PARKING_JVM_OPTS -cp "$BASE_DIR/etc/;$PARKING_LIBS" -Dfile.encoding=UTF-8 -Dapp.home="$BASE_DIR" com.microinfra.parking.ParkingApplication microinfra.parking >/dev/null 2>&1 &

PIDS=$(pid)
echo "parking servie($PIDS) started，you can check the state from log"
