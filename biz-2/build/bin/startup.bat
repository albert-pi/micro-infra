...
rem Perf options
set PARKING_JVM_OPTS=^
	-server ^
	-XX:MaxInlineLevel=15 ^
	-Djava.awt.headless=true

rem GC options
set PARKING_JVM_OPTS=%PARKING_JVM_OPTS% ^
...
rem set PARKING_JVM_OPTS=%PARKING_JVM_OPTS% -Xss16m

rem Uncomment to set preference to IPv4 stack.
rem set PARKING_JVM_OPTS=%PARKING_JVM_OPTS% -Djava.net.preferIPv4Stack=true

rem Uncomment to enable reverse DNS lookup.
rem set PARKING_JVM_OPTS=%PARKING_JVM_OPTS% -Dsun.net.spi.nameservice.provider.1=default -Dsun.net.spi.nameservice.provider.2=dns,sun

rem Remote debugging (JPDA). Uncomment and change if remote debugging is required.
rem set PARKING_JVM_OPTS=%PARKING_JVM_OPTS% -agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=8787 

set PARKING_LIBS=%BASE_DIR%\lib\*

for /F %%F in ('dir /A:D /b "%PARKING_LIBS%"') do (
	if not "%%F" == "optional" (
		set PARKING_LIBS=%PARKING_LIBS%;%BASE_DIR%\lib\%%F\*
	)
)
...
%JAVA_COMMAND% %PARKING_JVM_OPTS% -cp "%BASE_DIR%\etc\;%PARKING_LIBS%" -Dfile.encoding=UTF-8 -Dapp.home="%BASE_DIR%" com.microinfra.parking.ParkingApplication microinfra.parking %*
