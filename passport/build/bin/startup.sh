...
pid() {
  echo "$(ps -ef | grep microinfra.passport | grep java | grep -v grep | awk '{print $2}')"
}
...
if [ -n "$(pid)" ]; then
  echo "passport service already started: $(pid)"
  exit 1
fi
...
PASSPORT_JVM_OPTS="$PASSPORT_JVM_OPTS \
...
# GC options
PASSPORT_JVM_OPTS="$PASSPORT_JVM_OPTS -XX:+UnlockExperimentalVMOptions \
...
# PASSPORT_JVM_OPTS="${PASSPORT_JVM_OPTS} -Xss4m"

#
# Uncomment to set preference for IPv4 stack.
# PASSPORT_JVM_OPTS="${PASSPORT_JVM_OPTS} -Djava.net.preferIPv4Stack=true"

#
# Remote debugging (JPDA). Uncomment and change if remote debugging is required.
# PASSPORT_JVM_OPTS="-agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=8787 ${JVM_OPTS}"
...
do
    if [ -d $file ] ; then
        PASSPORT_LIBS=${PASSPORT_LIBS:-}$SEP$file/*
    fi
done
...
nohup $JAVA_CMD $PASSPORT_JVM_OPTS -cp "$BASE_DIR/etc/;$PASSPORT_LIBS" -Dfile.encoding=UTF-8 -Dapp.home="$BASE_DIR" com.microinfra.passport.PassportApplication microinfra.passport >/dev/null 2>&1 &

PIDS=$(pid)
echo "passport service($PIDS) started，you can check the state from log"
